...
int main (int argc, char **argv)
{
  ...
  int miter = 5,  mprec = 2;    
  // mprec is log2(min(width, height)/25) + 1, rounded down. 
  ...
  if (ReadPgm ("./seq/images/IM000.pgm", &I)){
  ...
  for (i = 0; i < nbimages; i++){ 
    ...
    sprintf (filename, "./seq/images/IM%03d.pgm", i);
   ...
   /* printf("Matrix data is: \n");
    int j;
    for (j = 0; j < I.rows/32; j++){
    	printf("%d, ", (int)I.data[j]);
    	if (j % 32 == 0 && j != 0)
    		printf("\n");
    }*/
