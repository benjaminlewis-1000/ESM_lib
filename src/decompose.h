...
decompose_return computeHomographyDecomposition(Eigen::Matrix3d H, Eigen::Matrix3d K){
	...
	#if(DEBUG)
		cout << "H after the K-1 * K mult: " << H << endl;
		cout << "K is "<< K << endl;
	#endif
	...
	#if(DEBUG)
		cout << "Median SVD is " << medianSVD << endl;
	#endif
...
// The largest diagonal value determines which method should be used for finding the ta and tb values. 
	...
	#if(DEBUG)
	Eigen::MatrixXd ta_sr1(3,1), tb_sr1(3,1);
	ta_sr1 << sr11, sr12 + sqrt(M_sr33), sr13 + er23 * sqrt( (M_sr22) );
	tb_sr1 << sr11, sr12 - sqrt(M_sr33), sr13 - er23 * sqrt( (M_sr22) );
	
	cout << "ta11 = " << ta_sr1 << endl << "ta2 = " << tb_sr1 << endl;
	#endif
